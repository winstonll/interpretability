...
parser.add_argument('--z_dim', dest='z_dim', type=int, default=5, help='dimension of latent')
...
# dataset and models
Dataset, img_shape, get_imgs, get_lbls = utils.get_dataset(dataset_name)
...
def enc_dec(img, lbl, is_training=True):
    # encode
    ...
    z_dim = z.shape[1]
    partial_z = tf.slice(z, [0, 1], [-1, z_dim-1])
    lbl_reshape = tf.reshape(lbl, (-1, 1))   # [?] to [?, 1]
    z = tf.concat([lbl_reshape, partial_z], 1)
...
lbl = tf.placeholder(tf.float32, [None])
z_sample = tf.placeholder(tf.float32, [None, z_dim])

# encode & decode
z_mu, z_log_sigma_sq, img_rec = enc_dec(img, lbl)
...
# sample
_, _, img_rec_sample = enc_dec(img, lbl, is_training=False)
...
# train
try:
    data_sample = dataset_val.get_next()
    img_ipt_sample = get_imgs(data_sample)
    img_lbl_sample = get_lbls(data_sample)
    
    z_partial_sample = np.random.normal(size=[100, z_dim-1])   # z[100, 1~zdim]
    z_lbl_sample = np.random.randint(10, size=[100, 1])        # z[100, 0], randomly generated label
    z_ipt_sample = np.concatenate((z_lbl_sample, z_partial_sample), axis=1)
    ...
    for ep in range(epoch):
        ...
        for batch in dataset:
            ...
            img_lbl = get_lbls(batch)

            # train D
            summary_opt, _, los, rec, kld = sess.run([summary, step, loss, rec_loss, kld_loss], feed_dict={img: img_ipt, lbl: img_lbl})
            summary_writer.add_summary(summary_opt, it)

            # display
            if (it + 1) % 100 == 0:
                print("Epoch: (%3d) (%5d/%5d), loss: (%5f), rec: (%5f), kld: (%5f)" % (ep, it_in_epoch, it_per_epoch, los, rec, kld))
            ...
            if (it + 1) % 1000 == 0:
                ...
                img_rec_opt_sample = sess.run(img_rec_sample, feed_dict={img: img_ipt_sample, lbl:img_lbl_sample})
